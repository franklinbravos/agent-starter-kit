...
0.3.0 - 2026/09/11
feat(personas): add SecOps Engineer (hands-on testing, findings, technical report) and SecOps Manager (business risk, management reporting, proposals) — two focused agents replacing a single generalist
feat(skills): add security-assessment (scope gate, black/gray/white-box modes, full lifecycle recon→retest), security-testing (per-asset catalogs and controlled-exploitation discipline), security-report (Kolivo technical + management HTML standard, zero-day convention, reproducible from zero), security-knowledge (read/refresh/grow loop), secops-engineer-self-review (PROOF) and secops-manager-self-review (CLEAR)
feat(knowledge): add knowledge/security base — sources, watchlist, techniques, lessons — so the agents evolve across engagements instead of restarting from zero
refactor(maestro): route security work to the SecOps Engineer/Manager and exempt reports from the code-review loop; document the scope-gate and engagement-mode requirements
docs(personas,skills,readme): register the two personas, five skills, and the knowledge base
chore(version): bump fork VERSION to 0.3.0
