...
This fork extends the original [ntorga/agent-starter-kit](https://github.com/ntorga/agent-starter-kit) with a **message queue system** that makes the Maestro resilient to user messages sent while it is mid-task. The original kit processes messages synchronously — anything you type while the agent is waiting for a sub-agent corrupts the session. This fork adds a FIFO queue (`.agents/skills/message-queue/SKILL.md`) that defers non-urgent messages and handles `/force` interruptions cleanly.

Differences from upstream:
- **`skills/message-queue/SKILL.md`** — file-based FIFO queue with mid-task semaphore, collation-resistant naming, `/force` command support, and stress-tested edge cases
- **`personas/maestro.md`** — updated playbook with queue-aware steps 0 (check), 6 (mark), 8 (clear), 9 (sweep)
- **`skills/boot/SKILL.md`** — initializes `.memory/queue/` during session startup
- **`skills/agent-memory/SKILL.md`** — logs queue events to session memory
