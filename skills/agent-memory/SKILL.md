...
- **Queue events** use actor `[maestro]` and follow the pattern `Queued message: "<preview>"` or `Processed queued message: "<preview>"`, where `<preview>` is the first 80 characters of the message. The Maestro records these when it enqueues or dequeues a message (uses: `skills/message-queue/SKILL.md`).
