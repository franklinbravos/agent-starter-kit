---
shortDescription: Security engagement methodology — scope gate, black/gray/white-box modes, full lifecycle.
...
3. **Knowledge load (step zero).** Read and follow `skills/security-knowledge.md` — load the watchlist and refresh intelligence before reconnaissance.
...
6. **Phase 3 — Vulnerability analysis.** Match the fingerprinted stack against `knowledge/security/watchlist.md` and vendor advisories; run the relevant checklist from `skills/security-testing.md`. For white-box, add the static review. Produce candidate findings with evidence.
...
10. **Phase 7 — Reporting.** Read and follow `skills/security-report.md`: the SecOps Engineer produces the technical report, the SecOps Manager produces the management report from the same findings.
...
12. **Knowledge write (at handoff).** Read and follow `skills/security-knowledge.md` — append lessons, techniques, and watchlist updates.
